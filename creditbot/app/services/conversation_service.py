...
# Intención de reiniciar la conversación (volver al menú principal)
RESTART_EXACT = frozenset(
    {"hola", "hi", "hello", "buenas", "reiniciar", "menu", "menú", "inicio", "cancelar", "0"}
)
RESTART_PHRASES = (
    "empezar de nuevo",
    "comenzar de nuevo",
...
def _clean_cedula(value: str) -> str:
    """Normaliza la c├®dula quitando espacios y guiones."""
    return value.strip().replace("-", "").replace(" ", "")


def _build_summary_data(user: dict[str, Any], request: dict[str, Any]) -> dict[str, Any]:
    """Construye un dict con los datos resumidos para confirmaci├│n del usuario."""
...
def _build_result_data(evaluation: dict[str, Any]) -> dict[str, Any]:
    """Construye un dict con los datos del resultado de la precalificaci├│n v2."""
    return {
...
def _reset_validation_failures(conversation_id: str) -> None:
    """Reinicia el contador de fallos de validaci├│n para una conversaci├│n."""
    _validation_failures.pop(conversation_id, None)


def _track_validation_failure(conversation_id: str) -> bool:
    """Incrementa el contador de fallos y retorna True si se super├│ el l├¡mite."""
...
def _request_handoff(
    ...
    """Deriva la conversaci├│n a un asesor humano y finaliza la conversaci├│n activa."""
...
def _handle_validation_failure(
    ...
    """Maneja un fallo de validaci├│n; si se supera el l├¡mite, deriva a asesor."""
...
def _is_restart_intent(text: str) -> bool:
    ...
    if normalized in RESTART_EXACT:
        return True
    return any(phrase in normalized for phrase in RESTART_PHRASES)


def _restart_conversation(conversation_id: str, user_id: str) -> str:
    """Finaliza la conversación actual y abre una nueva en el menú principal."""
    _reset_validation_failures(conversation_id)
    conversation_repository.finish_conversation(conversation_id)
    new_conversation = conversation_repository.create_conversation(user_id)
    new_conversation_id = new_conversation["id"]
    response = message_service.welcome_message()
    conversation_repository.update_state(new_conversation_id, MENU)
    conversation_repository.update_last_message(new_conversation_id, response)
    message_repository.save_outbound_message(new_conversation_id, user_id, response)
    return response
...
def process_message(phone: str, text: str, raw_payload: dict[str, Any] | None = None) -> str:
    ...
    response = ""
    next_state = state
    handled_restart = False

    if _is_restart_intent(normalized_text) and state not in {START, HANDOFF_REQUESTED, FINISHED}:
        if state == MENU:
            response = message_service.welcome_message()
            next_state = MENU
            handled_restart = True
        else:
            return _restart_conversation(conversation_id, user_id)

    if not handled_restart:
        if state == START:
            response = message_service.welcome_message()
            next_state = MENU
